...
test_that("expectations are created returned by insertExpectationsAddin()", {
  ...
  expect_equal(
    ...
    list(
      ...
      "# Testing side effects",
      "expect_warning(\n  xpectr::strip_msg(suppressMessages(msgs_warns_fn())),\n  xpectr::strip(\"hehe\"),\n  fixed = TRUE)",
      "expect_warning(\n  xpectr::strip_msg(suppressMessages(msgs_warns_fn())),\n  xpectr::strip(\"ohhh\"),\n  fixed = TRUE)",
      "expect_message(\n  xpectr::strip_msg(suppressWarnings(msgs_warns_fn())),\n  xpectr::strip(\"hihi\\n\"),\n  fixed = TRUE)",
      "expect_message(\n  xpectr::strip_msg(suppressWarnings(msgs_warns_fn())),\n  xpectr::strip(\"ihhh\\n\"),\n  fixed = TRUE)",
      "# Testing is NULL",
  ...
  expect_equal(
    ...
    list(
      "## Testing 'msgs_warns_fn()'                                                ####\n## Initially generated by xpectr",
      "# Testing side effects",
      "expect_warning(\n  suppressMessages(msgs_warns_fn()),\n  \"hehe\",\n  fixed = TRUE)",
      "expect_warning(\n  suppressMessages(msgs_warns_fn()),\n  \"ohhh\",\n  fixed = TRUE)",
      "expect_message(\n  suppressWarnings(msgs_warns_fn()),\n  \"hihi\\n\",\n  fixed = TRUE)",
      "expect_message(\n  suppressWarnings(msgs_warns_fn()),\n  \"ihhh\\n\",\n  fixed = TRUE)",
      "# Testing is NULL",
      "expect_true(\n  is.null(xpectr::suppress_mw(msgs_warns_fn())))",
      "## Finished testing 'msgs_warns_fn()'                                       ####"
  ...
  expect_equal(
    ...
    list(
      ...
      paste0("expect_equal(\n  xpectr::smpl(long_df[[\"a\"]], n = 30),\n ",
             ...
             "506,\n    0.45774, 0.71911, 0.93467, 0.25543, 0.97823, 0.117",
             "49, 0.475,\n    0.90403, 0.13871, 0.98889, 0.94667, 0.08244,",
             " 0.51421, 0.3902,\n    0.44697, 0.836, 0.7376, 0.81106, 0.00",
             "395, 0.83292, 0.00733,\n    0.20766, 0.61178),\n  tolerance ",
             "= 1e-4)"),
      paste0("expect_equal(\n  xpectr::smpl(long_df[[\"b\"]], n = 30),\n ",
             " c(0.43577, 0.03743, 0.43175, 0.95758, 0.88775, 0.63998, 0.6",
             "1884,\n    0.33343, 0.34675, 0.39849, 0.78469, 0.67728, 0.17",
             "126, 0.26109,\n    0.67561, 0.98282, 0.75954, 0.56649, 0.849",
             "69, 0.18947, 0.27129,\n    0.6932, 0.24054, 0.04299, 0.14048",
             ", 0.19741, 0.71936, 0.00788,\n    0.37549, 0.00157),\n  tole",
             "rance = 1e-4)"),
      "# Testing column names",
      "expect_equal(\n  names(long_df),\n  c(\"a\", \"b\"),\n  fixed = TRUE)",
      "# Testing column classes",
